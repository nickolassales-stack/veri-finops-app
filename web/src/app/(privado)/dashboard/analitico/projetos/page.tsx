...
/**
 * /dashboard/analitico/projetos -- por projeto do Public Cloud.
 *
 * ---------------------------------------------------------------------------
...
export const metadata = { title: "Custos OVH por projeto" };

export default async function ProjetosOvhPage({
  searchParams,
}: {
  ...
  if (params.provider !== "ovh") {
    ...
    redirect(`/dashboard/analitico/projetos?${busca.toString()}`);
  }

  return (
    <Suspense fallback={<CarregandoLinhas linhas={10} />}>
      <PainelAnaliticoOvh aba="projetos" />
    </Suspense>
  );
}
