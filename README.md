...
| [scripts/migrations/009-sync-runs-source-job.sql](scripts/migrations/009-sync-runs-source-job.sql) | `ovh_sync_runs.source` passa a aceitar `job`. Encontrada em producao: o worker gravava `job`, o CHECK recusava, e a coleta funcionava **sem ficar registrada** — sucesso invisivel na tela. **Reversivel** ([rollback](scripts/migrations/009-sync-runs-source-job-rollback.sql)), reclassificando as linhas como `manual` |
...
| [scripts/onboard-cur-account.sh](scripts/onboard-cur-account.sh) | Onboarding de conta AWS no Athena: valida S3, cria tabela e partições. **Não altera a view** — ver [docs/onboard-nova-conta.md](docs/onboard-nova-conta.md) |
